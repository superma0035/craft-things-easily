...
import { sessionTokenManager } from '@/lib/supabaseClient';
...
export const createClientWithSessionHeaders = (sessionToken?: string) => {
  // Prefer explicit token, then localStorage, then in-memory manager
  const token = sessionToken || localStorage.getItem('session_token') || sessionTokenManager.getSessionToken();
...
export const getSessionToken = (): string | null => {
  return localStorage.getItem('session_token') || sessionTokenManager.getSessionToken();
};

// Set session token in localStorage
export const setSessionToken = (token: string): void => {
  localStorage.setItem('session_token', token);
  sessionTokenManager.setSessionToken(token);
};
